...
import com.hoperlady.Pojo.MaterialEditextValue;
...
public class MaterialAddNewAdapter extends BaseAdapter {
    ...
    public View getView(final int aPosition, View convertView, ViewGroup parent) {
        ...
        if (myListItems.size() > 0) {
            MyTaskDetails.aOKLAY.setVisibility(View.GONE);
            MyTaskDetails.aCancelLAY.setVisibility(View.GONE);
            ...
            public void onClick(View v) {
                ...
                for (int i = 0; i < item_add.size(); i++) {
                    ...
                    if (toolname.equalsIgnoreCase("name") || toolname.equalsIgnoreCase("")) {
                        Toast.makeText(myContext, "Enter the Toolname", Toast.LENGTH_LONG).show();
                        break outerloop;
                    } else {
                    ...
                    if (toolcost.equalsIgnoreCase("cost") || toolcost.equalsIgnoreCase("")) {
                        Toast.makeText(myContext, "Enter the Toolcost", Toast.LENGTH_LONG).show();
                        break outerloop;
                    } else {
...
//            tool_cost.remove(position);
            if (position == myListItems.size() - 1) {
                MyTaskDetails.aOKLAY.setVisibility(View.VISIBLE);
                MyTaskDetails.aCancelLAY.setVisibility(View.VISIBLE);
                if (position == 0) {
    ...
    private void Submitmaterialfees(ArrayList<Materialcostsubmitpojo> arrayList) {
        ...
        MyTaskDetails.SubmitMaterialFees(arrayList, myContext);
    }
}
